      ...
      for (j = 0; j < zdom[ndom].mdim; j++)
	{
	  ...
	  if (zdom[ndom].log_linear == 1)
		...
		z_offset = zdisk (geo.diskrad);
	      ...
	      dz = (geo.rmax - z_offset) / (zdom[ndom].mdim - 4);
	      if (j == 0)
		w[n].x[2] = 0;
	      else if (j == 1)
	    ...
	    {			//logarithmic intervals
	      ...
	      dlogr = (log10 (geo.rmax / zdom[ndom].xlog_scale)) / (zdom[ndom].ndim - 3);
	      if (i == 0)
		{
		  w[n].x[0] = r = 0.0;
		...
		z_offset = zdisk (geo.diskrad);
	      ...
	      dlogz =
		(log10 ((geo.rmax - z_offset) / zdom[ndom].zlog_scale)) / (zdom[ndom].mdim - 4);
	      if (j == 0)
		{
		  w[n].x[2] = 0;
		  w[n].xcen[2] = 0.5 * z_offset;
     ...
     WindPtr w;
{
  ...
  for (i = 0; i < zdom[ndom].ndim - 1; i++)
      ...
      for (j = 0; j < zdom[ndom].mdim - 1; j++)
	{
	  ...
	  n = i * zdom[ndom].mdim + j;
	  dz = w[n + zdom[ndom].mdim].x[2] - w[n].x[2];	// change in z for one step in rho
	  drho = w[n + zdom[ndom].mdim].x[0] - w[n].x[0];
  ...
  for (i = 0; i < zdom[ndom].ndim; i++)
    {
      wind_x[i] = w[i * zdom[ndom].mdim].x[0];
      wind_midx[i] = w[i * zdom[ndom].mdim].xcen[0];
    }
...
/***********************************************************
 ...
 History:
	...
	11aug	ksl	70b - Added ability to get volumes for multiple
			components
	1508	ksl	Modified to accommodate multiple domains
     ...
     int icomp;			// The component for which we want the volume
{
  ...
  for (i = 0; i < zdom[ndom].ndim - 1; i++)
      ...
      for (j = 0; j < zdom[ndom].mdim - 1; j++)
	{
	  ...
	  if (rmin > w[n + 1].x[0])
	    rmin = w[n + 1].x[0];
	  if (rmin > w[n + zdom[ndom].mdim].x[0])
	    rmin = w[n + zdom[ndom].mdim].x[0];
	  if (rmin > w[n + zdom[ndom].mdim + 1].x[0])
	    rmin = w[n + zdom[ndom].mdim + 1].x[0];

	  rmax = w[n].x[0];
	  if (rmax < w[n + 1].x[0])
	    rmax = w[n + 1].x[0];
	  if (rmax < w[n + zdom[ndom].mdim].x[0])
	    rmax = w[n + zdom[ndom].mdim].x[0];
	  if (rmax < w[n + zdom[ndom].mdim + 1].x[0])
	    rmax = w[n + zdom[ndom].mdim + 1].x[0];

	  zmin = w[n].x[2];
	  if (zmin > w[n + 1].x[2])
	    zmin = w[n + 1].x[2];
	  if (zmin > w[n + zdom[ndom].mdim].x[2])
	    zmin = w[n + zdom[ndom].mdim].x[2];
	  if (zmin > w[n + zdom[ndom].mdim + 1].x[2])
	    zmin = w[n + zdom[ndom].mdim + 1].x[2];

	  zmax = w[n].x[2];
	  if (zmax < w[n + 1].x[2])
	    zmax = w[n + 1].x[2];
	  if (zmax < w[n + zdom[ndom].mdim].x[2])
	    zmax = w[n + zdom[ndom].mdim].x[2];
	  if (zmax < w[n + zdom[ndom].mdim + 1].x[2])
	    zmax = w[n + zdom[ndom].mdim + 1].x[2];
	  ...
	  for (r = rmin + dr / 2; r < rmax; r += dr)
	      ...
	      for (z = zmin + dz / 2; z < zmax; z += dz)
		{
		  ...
		  x[2] = z;
		  if (bilin
		      (x, w[i * zdom[ndom].mdim + j].x, w[i * zdom[ndom].mdim + j + 1].x,
		       w[(i + 1) * zdom[ndom].mdim + j].x, w[(i + 1) * zdom[ndom].mdim + j + 1].x,
		       &f, &g) == 0)
		    {
		      kk++;
		      if (where_in_wind (ndom, x) == W_ALL_INWIND)
			{
			  volume += r;
			  jj++;
			}
...
/***********************************************************
 ...
 History:
	...
	13sep	nsh	76b -- Modified calls to fraction to take account
			of new modes
	15aug	ksl	Modifiend for multiple domains
     ...
     double *fx, *fz;
{
  ...
  if (ichoice == 0)
    {
      fraction (rho, wind_x, zdom[ndom].ndim, &i, fx, 0);
      fraction (z[2], wind_z_var[i], zdom[ndom].mdim, &j, fz, 0);	// This should get one close
    }
  else
    {
      fraction (rho, wind_midx, zdom[ndom].ndim, &i, fx, 0);
      fraction (z[2], wind_midz_var[i], zdom[ndom].mdim, &j, fz, 0);	// This should get one close
    }
  ...
  if (rho > wind_x[zdom[ndom].ndim - 1] || z[2] > wind_z_var[i][zdom[ndom].mdim - 1])
    {
     ...
     double x[];		// Returned position
{
  ...
  if (rmin > wmain[n + 1].x[0])
    rmin = wmain[n + 1].x[0];
  if (rmin > wmain[n + zdom[ndom].mdim].x[0])
    rmin = wmain[n + zdom[ndom].mdim].x[0];
  if (rmin > wmain[n + zdom[ndom].mdim + 1].x[0])
    rmin = wmain[n + zdom[ndom].mdim + 1].x[0];

  rmax = wmain[n].x[0];
  if (rmax < wmain[n + 1].x[0])
    rmax = wmain[n + 1].x[0];
  if (rmax < wmain[n + zdom[ndom].mdim].x[0])
    rmax = wmain[n + zdom[ndom].mdim].x[0];
  if (rmax < wmain[n + zdom[ndom].mdim + 1].x[0])
    rmax = wmain[n + zdom[ndom].mdim + 1].x[0];

  zmin = wmain[n].x[2];
  if (zmin > wmain[n + 1].x[2])
    zmin = wmain[n + 1].x[2];
  if (zmin > wmain[n + zdom[ndom].mdim].x[2])
    zmin = wmain[n + zdom[ndom].mdim].x[2];
  if (zmin > wmain[n + zdom[ndom].mdim + 1].x[2])
    zmin = wmain[n + zdom[ndom].mdim + 1].x[2];

  zmax = wmain[n].x[2];
  if (zmax < wmain[n + 1].x[2])
    zmax = wmain[n + 1].x[2];
  if (zmax < wmain[n + zdom[ndom].mdim].x[2])
    zmax = wmain[n + zdom[ndom].mdim].x[2];
  if (zmax < wmain[n + zdom[ndom].mdim + 1].x[2])
    zmax = wmain[n + zdom[ndom].mdim + 1].x[2];
